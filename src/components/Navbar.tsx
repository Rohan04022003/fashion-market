        ...
        <header className="sticky top-0 z-50 backdrop-blur bg-background/95 supports-[backdrop-filter]:bg-background/80">
            <div className="flex justify-between items-center px-2 sm:px-6 md:px-12 lg:px-16 h-16">
                ...
                <nav className="hidden lg:flex items-center gap-2">
                    {Navlinks.map((link, index) => (
                        <Link
                            key={index}
                            to={link.path}
                            className={`px-2 py-2 cursor-pointer font-medium text-gray-500 ${active === link.path
                                    ? 'text-orange-500'
                                    : ""
                                }`}
                        >
                ...
                <div className="flex items-center gap-4">
                    ...
                    <button className={`lg:hidden transition-all duration-500 ${menuOpen ? "rotate-90": "rotate-0"}`} onClick={() => setMenuOpen(!menuOpen)}>
                        {menuOpen ? (
